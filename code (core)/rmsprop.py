...
class RMSPropOptimizer:
    def __init__(self, model, learning_rate=0.001, n_epochs=100, batch_size=32, decay_rate=0.9, epsilon=1e-8):
        self.model = model
        ...
        for epoch in range(self.n_epochs):
            ...
            for i in range(0, n_samples, self.batch_size):
                ...
                for j, (p, g) in enumerate(zip(params, grads)):
                    ...
                    adjusted_lr = self.learning_rate / (np.sqrt(self.Eg[j]) + self.epsilon)
...
def main():
    ...
    print(f"--- [Spambase] 使用RMSProp训练 {MODEL_TYPE.upper()} ---")
    ...
    if X_train is not None:
        ...
        optimizer = RMSPropOptimizer(model, learning_rate=0.01)
    ...
    print(f"\n--- [Ads] 使用RMSProp训练 {MODEL_TYPE.upper()} ---")
