...
class AdamOptimizer:
    def __init__(self, model, learning_rate=0.001, n_epochs=100, batch_size=32, beta1=0.9, beta2=0.999, epsilon=1e-8):
        self.model = model
        ...
        self.t = 0
        # 初始化一阶和二阶矩
        self.m = [np.zeros_like(p) for p in self.model.get_params()]
        self.v = [np.zeros_like(p) for p in self.model.get_params()]
        ...
        for epoch in range(self.n_epochs):
            ...
            for i in range(0, n_samples, self.batch_size):
                ...
                for j, (p, g) in enumerate(zip(params, grads)):
                    ...
                    updated_params.append(p - update_val)
...
def main():
    ...
    print(f"--- [Spambase] 使用Adam训练 {MODEL_TYPE.upper()} ---")
    ...
    if X_train is not None:
        ...
        optimizer = AdamOptimizer(model, learning_rate=0.01)
