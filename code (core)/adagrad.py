...
class AdaGradOptimizer:
    def __init__(self, model, learning_rate=0.01, n_epochs=100, batch_size=32, epsilon=1e-8):
        self.model = model
        ...
        # 初始化梯度平方累加器
        self.G = [np.zeros_like(p) for p in self.model.get_params()]
        ...
        for epoch in range(self.n_epochs):
            ...
            for i in range(0, n_samples, self.batch_size):
                ...
                for j, (p, g) in enumerate(zip(params, grads)):
                    self.G[j] += g**2
                    adjusted_lr = self.learning_rate / (np.sqrt(self.G[j]) + self.epsilon)
                    updated_params.append(p - adjusted_lr * g)
...
def main():
    ...
    print(f"--- [Spambase] 使用AdaGrad训练 {MODEL_TYPE.upper()} ---")
    ...
    if X_train is not None:
        ...
        optimizer = AdaGradOptimizer(model, learning_rate=0.1)
    ...
    print(f"\n--- [Ads] 使用AdaGrad训练 {MODEL_TYPE.upper()} ---")
